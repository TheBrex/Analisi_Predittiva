...
Tutte le informazioni sui dati e sulle loro misurazioni sono disponibili sul sito del [https://nrfa.ceh.ac.uk/](National%20River%20Flow%20Archive).

```{r dataRead}
ukrivers <- read.csv("datasets/ukrivers.csv",header=TRUE)
plot(ukrivers[,6:12])
```
...
e notiamo che ora la relazione sembra essere lineare e più regolare. Il modello di partenza è $$\text{lqmed}_i = \beta_0 + \beta_1 \text{larea} + \epsilon_i$$
...
-   Si usi un algoritmo (o più di un algoritmo) per selezionare in maniera automatica quali variabili inserire nel modello (ad esempio confrontando se modelli diversi vengono proposti usando le variabili originali o le variabili trasformate)
    ...
    sel_forw <- step(object = lfit0_null, direction = "both", 
                     scope = list(lower=lfit0_null, upper = lfit0_all), k=2)
...
-   
    ...
    ```{r}
    lfit<- lm(lqmed ~  bcqmed  + I(BFI^2) + log(ALTBAR), data = tukrivers)
    ```
